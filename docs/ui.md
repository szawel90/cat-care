...
Shared UI components also have isolated stories, interaction tests and automated accessibility checks in Storybook. Use the real components, tokens and catalogs, review both themes and languages, then verify the complete application flow. See [Component workbench](./storybook.md) for commands and conventions.
